...
def fru(incoming, num_stats, freqs, freqs_mask, output_dims, recur_dims, seq_len, activation=tf.nn.relu,
         dropout=None, bias=True, weights_init=None, forget_bias=1.0,
         return_seq=False, return_state=False, initial_state=None,
         dynamic=False, trainable=True, restore=True, reuse=False,
         scope=None, name="FRU"):
    cell = FRUCell(num_stats=num_stats, freqs=freqs, freqs_mask=freqs_mask, output_dims=output_dims, recur_dims=recur_dims, seq_len=seq_len, activation=activation)
    x = _rnn_template(incoming, cell=cell, dropout=dropout,
                      return_seq=return_seq, return_state=return_state,
                      initial_state=initial_state, dynamic=dynamic,
                      scope=scope, name=name)
...
def sru(incoming, num_stats, mavg_alphas, output_dims, recur_dims, activation=tf.nn.relu,
         dropout=None, bias=True, weights_init=None, forget_bias=1.0,
         return_seq=False, return_state=False, initial_state=None,
         dynamic=False, trainable=True, restore=True, reuse=False,
         scope=None, name="SRU"):
    cell = SimpleSRUCell(num_stats=num_stats, mavg_alphas=mavg_alphas, output_dims=output_dims, recur_dims=recur_dims, activation=activation)
    x = _rnn_template(incoming, cell=cell, dropout=dropout,
                      return_seq=return_seq, return_state=return_state,
                      initial_state=initial_state, dynamic=dynamic,
                      scope=scope, name=name)
